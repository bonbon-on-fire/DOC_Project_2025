...
export class RendererRegistry {
  ...
  register<T extends MessageDto>(
    ...
    if (this.renderers.has(messageType) && !force) {
      throw new Error(`Renderer for message type '${messageType}' is already registered. Use force=true to override.`);
    }

    // Validate that the renderer's messageType matches the registration key
    if (renderer.messageType !== messageType) {
      console.warn(`Renderer messageType '${renderer.messageType}' does not match registration key '${messageType}'`);
    }
