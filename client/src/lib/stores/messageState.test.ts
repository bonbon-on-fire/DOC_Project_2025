...
describe('Message State Management', () => {
  ...
  describe('updateMessageState', () => {
    ...
    it('should validate messageId parameter', () => {
      expect(() => updateMessageState('', { expanded: true })).toThrow('Message ID must be a non-empty string');
      expect(() => updateMessageState('   ', { expanded: true })).toThrow('Message ID must be a non-empty string');
      expect(() => updateMessageState(null as any, { expanded: true })).toThrow('Message ID must be a non-empty string');
    });

    it('should validate state updates', () => {
      expect(() => updateMessageState('msg-1', { expanded: 'invalid' as any })).toThrow('expanded must be a boolean');
      expect(() => updateMessageState('msg-1', { renderPhase: 'invalid' as any })).toThrow('renderPhase must be one of');
    });
  ...
  describe('cleanupMultipleMessageStates', () => {
    ...
    it('should validate all messageIds', () => {
      expect(() => cleanupMultipleMessageStates(['msg-1', ''])).toThrow('Message ID must be a non-empty string');
    });
  });
...
describe('Performance and Memory Management', () => {
  it('should not leak memory with repeated operations', () => {
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      if (i % 100 === 0) {
        cleanupMultipleMessageStates(
          Array.from({ length: 50 }, (_, j) => `msg-${i - j}`)
        );
      }
    }
