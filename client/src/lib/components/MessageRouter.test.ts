...
describe('MessageRouter Component Logic', () => {
  beforeEach(() => {
    ...
    // Set up mock implementations for renderer functions
    vi.mocked(getRenderer).mockImplementation((messageType: string) => {
      if (messageType === 'text') {
        return { messageType: 'text' };
      } else if (messageType === 'unknown-type' || messageType === 'unknown' || messageType === 'invalid-type' || !messageType || messageType.trim() === '') {
        return { messageType: 'fallback' };
      }
      return { messageType: 'fallback' };
    });
  });
