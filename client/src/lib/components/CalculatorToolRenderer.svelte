...
>
	...
	<div class="calculator-display rounded-lg p-6">
		<div class="text-center">
			<!-- Expression -->
			<div class="calc-expression mb-3 text-3xl font-mono"
				...
				<div class="calc-loading-container pt-3">
					<div class="calc-loading flex items-center justify-center space-x-2">
						<div class="flex space-x-1">
							<div class="calc-loading-dot h-2 w-2 animate-bounce rounded-full" style="animation-delay: 0ms"></div>
							<div class="calc-loading-dot h-2 w-2 animate-bounce rounded-full" style="animation-delay: 150ms"></div>
							<div class="calc-loading-dot h-2 w-2 animate-bounce rounded-full" style="animation-delay: 300ms"></div>
						</div>
	...
	{#if expanded && (toolCallPair.toolCall.function_args || toolCallPair.toolCall.args)}
		<details class="mt-4">
			...
			<div class="mt-2 rounded p-2 text-xs font-mono">
				<pre class="whitespace-pre-wrap">{JSON.stringify(toolCallPair.toolCall.function_args || toolCallPair.toolCall.args, null, 2)}</pre>
			</div>
		</details>
	{/if}
</div>
