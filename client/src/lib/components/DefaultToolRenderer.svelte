<script lang="ts">
	...
	function getToolCallId(pair: typeof toolCallPair): string {
		return pair.toolCall.tool_call_id || 
			   pair.toolCall.id || 
			   `tool_${pair.toolCall.index || index}`;
	}
	
	// Check if result is an error
	function hasError(pair: typeof toolCallPair): boolean {
		return !!pair.toolResult && 
	...
	function formatResult(result: string): {isJson: boolean, formatted: string, isMarkdown: boolean} {
	...
	$: formattedResult = toolCallPair.toolResult ? formatResult(toolCallPair.toolResult.result) : null;
</script>
			...
			<pre class="args-content"><!--
				-->{#each formatArgsAsYaml(argsToFormat) as segment}<!--
					...
					-->{:else if segment.type === 'string'}<!--
						--><span class="text-green-600 dark:text-green-400">{segment.content}</span><!--
					-->{:else if segment.type === 'number'}<!--
						--><span class="text-blue-600 dark:text-blue-400">{segment.content}</span><!--
					-->{:else if segment.type === 'boolean'}<!--
						--><span class="text-purple-600 dark:text-purple-400">{segment.content}</span><!--
					-->{:else if segment.type === 'null'}<!--
						--><span class="text-gray-500 dark:text-gray-500">{segment.content}</span><!--
					-->{:else}<!--
		...
		<div class="tool-result" class:error={isError}>
			...
			{#if formattedResult}
				{#if formattedResult.isJson}
					<pre class="result-content json" data-testid="tool-result">{formattedResult.formatted}</pre>
