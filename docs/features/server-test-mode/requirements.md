...
#### Acceptance Criteria:
  1. [ ] WHEN `ASPNETCORE_ENVIRONMENT=Test` THEN the server loads `appsettings.Test.json`.
  2. [ ] WHEN in Test env THEN the constructed `HttpClient` for LLM uses a Test `HttpMessageHandler` (no outbound HTTP).
...
#### Acceptance Criteria:
  1. [ ] WHEN a streaming request is made THEN chunks use `object: "chat.completion.chunk"` with `choices[0].delta.content` for text and `choices[0].delta.reasoning` for reasoning.
  2. [ ] [DONE] is sent as the final line after a finish chunk (`finish_reason: "stop"`).
  3. [ ] No non-streaming fallback calls occur in Test mode.
...
#### Acceptance Criteria:
  1. [ ] WHEN the latest user message contains "\nReason:" THEN the handler streams reasoning updates first (via `delta.reasoning`) before any text chunks.
  2. [ ] Reasoning visibility is treated as visible (no encryption) for test-mode streams.
  3. [ ] The reasoning content reflects the text following "Reason:" (may be chunked by words and paced).
...
#### Acceptance Criteria:
  1. [ ] WHEN streaming text THEN total lorem ipsum word count is between 5 and 500 determined by a stable hash of the user message.
  2. [ ] Chunks are emitted at ~50 words per second overall, with simple word-based chunking.
  3. [ ] Chunk sizes may vary (e.g., 3–10 words per chunk) but maintain the aggregate pacing.
...
#### Acceptance Criteria:
  1. [ ] AFTER lorem/Reasoning streaming completes THEN the verbatim user message is appended to each stream (reasoning and text), chunked by words.
  2. [ ] The echoed content does not count toward the lorem length computation.

### Requirement 6: Termination Semantics
- **User Story**: As a client, I can reliably detect stream completion.

#### Acceptance Criteria:
  1. [ ] A proper finish chunk with `finish_reason: "stop"` is sent before the terminator.
  2. [ ] The stream ends with a line containing only `[DONE]`.

### Requirement 7: Database and Networking in Test
- **User Story**: As a test operator, I want isolation from dev runs.

#### Acceptance Criteria:
  1. [ ] In Test env the server uses EF Core InMemory provider and ensures a clean DB on startup.
  2. [ ] The server binds to HTTP on localhost port 5099 and disables HTTPS redirection.
...
#### Acceptance Criteria:
1. [ ] WHEN the latest user message contains a block delimited by `<|instruction_start|>` and `<|instruction_end|>` THEN only JSON inside that block is parsed; text outside is ignored for generation.
2. [ ] The JSON structure supports:
...
3. [ ] FOR EACH `messages[i]` an independent stream is emitted with `choices[0].index = i`.
4. [ ] FOR `text_message` items, if `reasoning.length` is present, stream reasoning first, then text; lengths exclude the `id_message` contribution.
5. [ ] FOR `tool_call` items, emit OpenAI-compatible `delta.tool_calls` updates with correct indices and progressive emission of `name` and `args`.
6. [ ] IF the instruction block is absent or invalid JSON, fall back to legacy Test behavior (hash-based lorem with optional `\nReason:` trigger).
